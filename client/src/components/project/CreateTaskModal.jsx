...
const CreateTaskModal = ({ projectID, latestSprint, setIsCreateTaskOpen, isCreateTaskOpen }) => {
  ...
  return (
    <dialog className="relative z-50 flex flex-col p-10 rounded-xl" ref={ dialogRef }>
      <button className='absolute top-3 right-3 hover:text-red-500 transition-none' onClick={() => setIsCreateTaskOpen(false)}>
      ...
      <h2 className='font-title font-bold text-3xl mb-5'>Crear tasca</h2>
      <label className='font-subtitle font-bold text-2xl' htmlFor='new-task-name'>Títol</label>
      <input ref={ newTaskNameRef } className=' font-body font-medium text-lg border-2 border-dark-primary-bg rounded-md outline-none' type="text" name="new-task-name" id="new-task-name" />
      ...
      <textarea className=' font-body font-medium text-lg border-2 border-dark-primary-bg rounded-md outline-none' name="new-task-description" id="new-task-description" cols="30" rows="10"></textarea>
